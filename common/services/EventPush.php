...
class EventPush extends \yii\base\Component
{
    use PublicMethodTrait;

    //设备黑名单检测
    public static function deviceProbing($event){
        if(!empty($event->orderId)){
            BlacklistDashboard::relieveDevice($event->orderId);
        }
    }
    ...
    public static function sendOrderGrapResult($event){
        $orderInfo = Order::find()->select(['id','passenger_info_id','driver_id','order_start_time','start_address','end_address',
            'start_longitude','start_latitude','end_longitude','end_latitude','mapping_number','user_feature','service_type'])->where(['id'=>$event->orderId])->asArray()->one();
        ...
        if ($event->extInfo['code'] == 0){//抢单成功
            ...
            $startTime = date("Y-m-d H:i", strtotime($orderInfo['order_start_time']));
            //取乘客尾号
            $phone = $result[0]['phone'] ? substr($result[0]['phone'], 7, 11) : '';
            if ($orderInfo['service_type'] == 5 || $orderInfo['service_type'] == 6){
                $data['content'] = "包车单抢单成功，".$startTime."上车点".$orderInfo['start_address']."乘客尾号".$phone.",请合理安排接乘时间。";
            }else{
                $data['content'] = "预约单抢单成功，".$startTime."乘客尾号".$phone.",从".$orderInfo['start_address']."到".$orderInfo['end_address']."的订单抢单成功，请合理安排接乘时间。";
            }
            ...
            $messageType = 105;
            $data = array('message'=>'抢单失败','content'=>'预约单抢单失败。');
    ...
    static public function sendOrderGrapToPassenger($event){
        if ($event->extInfo['code'] == 0){
            $orderInfo = Order::find()->select(['id','passenger_info_id','driver_id','car_id','order_start_time','start_address','end_address',
                'start_longitude','start_latitude','end_longitude','end_latitude','other_phone','order_type','mapping_number','other_mapping_number'])->where(['id'=>$event->orderId])->asArray()->one();
            ...
            if ($orderInfo['order_type'] == 1){//给自己叫车
                ...
                $data = [
                    ...
                    'phone' => $orderInfo['mapping_number'] ? $orderInfo['mapping_number'] : $driverPhone[0]['phone'],
                    'customer_phone' => \Yii::$app->params['serviceManFixedPhone'],
                ];
                ...
                $phone = $passengerPhone[0]['phone'];
                $smsId = 'HX_0024';
                $start_time = Common::convertTimeToNaturalLanguage($orderInfo['order_start_time']);
                $data = [
                    'passenger_name' => $passengerName ?? '',
                    'travel_info' => $start_time."从".$orderInfo['start_address']."到".$orderInfo['end_address'],
                    'driver_name' => $driverInfo['driver_name'],
                    'plate_number' => $carInfo['plate_number'],
                    'phone' => $orderInfo['other_mapping_number'] ? $orderInfo['other_mapping_number'] : $driverPhone[0]['phone'],
                    'customer_phone' => \Yii::$app->params['serviceManFixedPhone'],
                ];
                Common::sendMessageNew($phone, $smsId, $data);
            }
    ...
    //去接乘客（201）--到达上车点（202）--乘客上车（203）--结束行程（204）--司机发起收款（有尾款）（205） --支付成功（206）--尾款支付成功（207）-- to乘客
    public static function orderStatusChange($event){
        ...
        //是否存业务消息
        if ($data['messageType'] == 202 || $data['messageType'] == 205 || $data['messageType'] == 206 || $data['messageType'] == 207){//存业务消息
        ...
        self::jpush(3, $jpushData, $msgType);//推送给乘客
    }
    ...
    public static function orderPaySuccess($event){
        ...
        //支付金额
        $payPrice = OrderPayment::find()->select(['final_price','coupon_reduce_price'])->where(['order_id'=>$orderInfo['id']])->asArray()->one();
        $totalPrice = strval(round(($payPrice['final_price']+$payPrice['coupon_reduce_price']),2));
        //取明文电话
        $result = Common::getPhoneNumber([['id'=>$orderInfo['passenger_info_id']]], 1);
        ...
        $data = array(
            'messageType' => $event->extInfo['messageType'] ?? '',
            'orderId' => $event->orderId ?? 0,
            'content' => $startTime.",乘客尾号".$phone.",从".$orderInfo['start_address']."到".$orderInfo['end_address']."的订单，已支付完成".$totalPrice."元。",
        );
        $jpushData = array(
            'sendId' => 'system',//发送者id
            'sendIdentity' => 1,//发送者身份
        ...
        self::jpush(3, $jpushData, 1);//推送给司机
    }
    ...
    public static function sendReceivePassenger($event){
        $orderInfo = Order::find()->select(['passenger_info_id','driver_id','car_id','passenger_phone','end_address','receive_passenger_longitude',
            'receive_passenger_latitude','start_longitude','start_latitude','end_longitude','end_latitude','service_type'])->where(['id'=>$event->orderId])->asArray()->one();
        $largeScreenDeviceCode = CarInfo::find()->select(['large_screen_device_code'])->where(['id'=>$orderInfo['car_id']])->scalar();
        $flightInfo = FlightNumber::find()->select(['flight_number','flight_date'])->where(['order_id'=>$event->orderId])->orderBy('create_time desc')->asArray()->one();

        //解密手机号
        $phone = Common::getPhoneNumber([['id'=>$orderInfo['passenger_info_id']]], 1);
        $data = array(
            'messageType' => 1010,
            'content' => ['欢迎乘坐逸品专车，请系好安全带，准备出发吧','现在向您展示行程信息，小逸还准备了丰富的娱乐内容和赠送流量哦'],
            ...
            'is_chartered' => ($orderInfo['service_type'] == 5 || $orderInfo['service_type'] == 6) ? 1 : 0,
        );
        $jpushData = array(
            'sendId' => 'system',//发送者id
            'sendIdentity' => 1,//发送者身份
        ...
        self::jpush(3, $jpushData); //推送给大屏
    }

    //乘客下车-to大屏
    public static function sendPassengerGetOff($event){
        $orderInfo = Order::find()->select(['driver_id','passenger_info_id','service_type'])->where(['id'=>$event->orderId])->asArray()->one();
        ...
        $data = array(
            ...
            'orderId' => $event->orderId ?? '',
            'is_chartered' => ($orderInfo['service_type'] == 5 || $orderInfo['service_type'] == 6) ? 1 : 0,
            'content' => ['目的地已到达，下车请带好随身物品，欢迎再次乘坐逸品专车']
        );
