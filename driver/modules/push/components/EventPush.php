<?php
namespace  driver\modules\push\components;
...
class EventPush extends \yii\base\Component
{
    ...
    public static function sendOrderGrapResult($event){
        $orderInfo = Order::find()->select(['id','passenger_info_id','driver_id','order_start_time','start_address','end_address',
            'start_longitude','start_latitude','end_longitude','end_latitude','mapping_number','user_feature'])->where(['id'=>$event->orderId])->asArray()->one();
        ...
        if ($event->extInfo['code'] == 0){//抢单成功
            ...
            if ($orderInfo['user_feature'] == 1){
                $data['content'] = "儿童用车预约单抢单成功，时间".$orderInfo['order_start_time']."乘客尾号".$phone.",从".$orderInfo['start_address']."到".$orderInfo['end_address']."的订单抢单成功，请合理安排接乘时间";
            }elseif ($orderInfo['user_feature'] == 2){
                $data['content'] = "女性用车预约单抢单成功，时间".$orderInfo['order_start_time']."乘客尾号".$phone.",从".$orderInfo['start_address']."到".$orderInfo['end_address']."的订单抢单成功，请合理安排接乘时间";
            }else{
                $data['content'] = "预约单抢单成功，时间".$orderInfo['order_start_time']."乘客尾号".$phone.",从".$orderInfo['start_address']."到".$orderInfo['end_address']."的订单抢单成功，请合理安排接乘时间";
            }
            ...
            $messageType = 105;
            $data = array('message'=>'抢单失败');

            if ($orderInfo['user_feature'] == 1){
                $data['content'] = "儿童用车预约单抢单失败";
            }elseif ($orderInfo['user_feature'] == 2){
                $data['content'] = "女性用车预约单抢单失败";
            }else{
                $data['content'] = "预约单抢单失败";
            }
