...
class AppMsgPlanController extends BossBaseController
{
    ...
    public function actionIndex(){
        ...
        if (!empty($smsAppList)){
            foreach ($smsAppList as $k=>$v){
                ...
                $peopleList =$this->getPeopleList($v['people_tag_id'], $v['show_type']);
                //取对应人群信息
                ...
                if (!empty($peopleList)){//循环人群，取对应的人的信息，拼接模板
                    foreach ($peopleList as $kk=>$vv){
                        ...
                        $requestData = array(
                            ...
                            'messageType' => 801,
                            'messageBody' => array('type'=>$sendType,'messageType'=>801,'content'=>$peopleTemplateContent,'smsImage'=>$ossFileUrl.$allTemplate[$v['app_template_id']]['sms_image'],'smsUrl'=>$allTemplate[$v['app_template_id']]['sms_url'],'smsSendAppId'=>$v['id'])
                        );
                        //调java消息服务推送消息
     ...
     */
    public function getPeopleList($peopleId, $showType){
        if ($peopleId > 0){
            ...
            if (!empty($peopleTag)){
                if ($peopleTag['tag_type'] == 1){//乘客
                    ...
                    $peopleList = PassengerInfo::find()->select(['id'])->where(['>','register_time',$regStart])->andWhere(['<','register_time',$regEnd])->asarray()->all();
                    ...
                    $peopleList = DriverInfo::find()->select(['id'])->where(['car_id'=>$carList])->asArray()->all();
                }
            }
        }else{//未选择人群id，取对应得全部用户
            if ($showType == 1){
                $peopleList = PassengerInfo::find()->select(['id'])->asarray()->all();
            }elseif ($showType == 2){
                $peopleList = DriverInfo::find()->select(['id'])->asArray()->all();
            }elseif ($showType == 4){
